    ...
    return (
        <form onSubmit={submit} className="space-y-4">
            ...
            <div className="space-y-2">
                ...
                {tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                        {tags.map((tag, idx) => (
                            <span key={idx} className="inline-flex items-center gap-1 px-3 py-1 bg-primary/10 text-primary text-sm rounded-full">
                                {tag}
                                <button
                                    type="button"
            ...
            <Button type="submit" className="w-full" disabled={submitting}>
                {submitting ? (editRecipe ? 'Updating...' : 'Adding...') : (editRecipe ? 'Update Recipe' : 'Add Recipe')}
            </Button>
        </form>
    )
}
